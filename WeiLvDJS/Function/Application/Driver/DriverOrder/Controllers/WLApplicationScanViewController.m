...
@interface WLApplicationScanViewController ()<UIAlertViewDelegate,AVCaptureMetadataOutputObjectsDelegate,UINavigationControllerDelegate, UIImagePickerControllerDelegate>{
...
@implementation WLApplicationScanViewController
...
-(void)setupTipTitleView{
    ...
    UIView*mask=[[UIView alloc]initWithFrame:CGRectMake(0, _maskView.y+_maskView.height, self.view.width, self.view.height)];
    ...
    //2.输入电子码
    UIButton * inputElectronicCodeBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    inputElectronicCodeBtn.frame = CGRectMake(0,self.view.height, ScaleW(200), self.view.height * 0.1);
    inputElectronicCodeBtn.center= CGPointMake(self.view.width/2 - 20, self.view.height /2 + ScaleH(170));
    [inputElectronicCodeBtn setImage:[UIImage imageNamed:@"Electronic-code"] forState:UIControlStateNormal];
    [inputElectronicCodeBtn setTitle:@"输入电子码" forState:UIControlStateNormal];
    [inputElectronicCodeBtn setTitleColor:[WLTools stringToColor:@"#ffffff"] forState:UIControlStateNormal];
    inputElectronicCodeBtn.titleEdgeInsets = UIEdgeInsetsMake(0, 20, 0, 0);
    [inputElectronicCodeBtn addTarget:self action:@selector(myCode) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:inputElectronicCodeBtn];
}
-(void)setupNavView{
    ...
    backBtn.frame = CGRectMake(20, 30, 0,0);
    ...
    titleLabel.frame = CGRectMake(0, 0, 100, 49);
    titleLabel.center= CGPointMake(self.view.width/2, 20+49/2.0);
    titleLabel.text = @"二维码";
    titleLabel.textColor = [WLTools stringToColor:@"#ffffff"];
    titleLabel.textAlignment = NSTextAlignmentCenter;
    [self.view addSubview:titleLabel];
    
    //3.历史记录
    UIButton * flashBtn=[UIButton buttonWithType:UIButtonTypeCustom];
    flashBtn.frame = CGRectMake(self.view.width-55,30, 0, 0);
   
    [flashBtn setBackgroundImage:[UIImage imageNamed:@"history"] forState:UIControlStateNormal];
     [flashBtn sizeToFit];
...
- (void)beginScanning
{
    ...
    output.metadataObjectTypes= @[AVMetadataObjectTypeQRCode,AVMetadataObjectTypeAztecCode,AVMetadataObjectTypePDF417Code,AVMetadataObjectTypeInterleaved2of5Code,AVMetadataObjectTypeUPCECode,AVMetadataObjectTypeCode39Code,AVMetadataObjectTypeCode39Mod43Code,AVMetadataObjectTypeEAN13Code,AVMetadataObjectTypeEAN8Code,AVMetadataObjectTypeCode93Code,AVMetadataObjectTypeCode128Code];
...
- (void)setNet:(NSString *)str
{
    [WLNetworkWriteOffHandler writeOffWithCode:str resultBlock:^(WLResponseType responseType, WLWriteOffObject *WLWriteOffObjectmodel, WLWriteOffObjectBuyUser *WLWriteOffObjectBuyUsermodel, NSString * message) {
        if (responseType == WLResponseTypeSuccess) {
            if ([WLWriteOffObjectmodel.is_passed_card isEqual:@"0"])
            {
                if (responseType == WLResponseTypeSuccess) {
                    ...
                    [self.navigationController pushViewController:nextVC animated:YES];//errSSLCertNotYetValid];
                }
            }
...
//        [self turnTorchOn:NO];
//    }
    WLWriteOffListViewController * nextVC = [[WLWriteOffListViewController alloc] init];
    [self.navigationController pushViewController:nextVC animated:YES];
...
-(void)myCode
{
    WLWriteOffViewController * nextVC = [[WLWriteOffViewController alloc] init];
    [self.navigationController pushViewController:nextVC animated:YES];
}
#pragma mark-> 开关闪光灯
