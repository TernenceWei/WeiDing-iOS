...
- (void)requestTouristGuideAuthenticationStatusWithResult:(void (^)(BOOL, TouristGuideOauthStatus, long long, NSString *))resultBlock
{
    ...
    NSString *companyID = [DEFAULTS objectForKey:@"currentCompanyID"];
...
- (void)submmitTouristGuidePersonalInfoWithInfo:(WLTouristGuideInfo *)info needAudits:(BOOL)needAudits result:(void (^)(BOOL, BOOL))resultBlock
{
    ...
    NSString *companyID = [DEFAULTS objectForKey:@"guideCompanyID"];
    NSString *usercompanyID = [DEFAULTS objectForKey:@"currentCompanyID"];
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        [manager POST:urlString parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData){
            ...
            if (info.IDFrontImg) {
                NSData *data1 = UIImageJPEGRepresentation(info.IDFrontImg, 0.2);
            ...
            if (info.IDBackImg) {
                NSData *data2 = UIImageJPEGRepresentation(info.IDBackImg, 0.2);
            ...
            if (info.cardIDFrontImg) {
                NSData *data3 = UIImageJPEGRepresentation(info.cardIDFrontImg, 0.2);
            ...
            if (info.cardIDBackImg) {
                NSData *data4 = UIImageJPEGRepresentation(info.cardIDBackImg, 0.2);
...
{
    return [WLUserTools getUserMobile];////[DEFAULTS objectForKey:@"user_mobile"];
}

- (NSString *)getUserPassword
{
    NSString *password = [WLUserTools getUserPassword];////[DEFAULTS objectForKey:@"userPassword"];
...
- (NSString *)getUserID
{
    return [WLUserTools getUserId];////[DEFAULTS objectForKey:@"user_id"];
}
...
{
    WlLog(@"%d",[NSThread currentThread].isMainThread);
    ...
    if (miniType != 0) {
        [params setObject:[NSString stringWithFormat:@"%ld",miniType] forKey:@"type_num"];
    }
