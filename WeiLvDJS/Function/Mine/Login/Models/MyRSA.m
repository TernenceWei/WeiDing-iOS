...
//    NSString *ret = base64_encode_data(data);
    ...
    for (int i=0; i < data.length; i++){
        [ret appendFormat:@"%0.2hhx", bytes[i]];
    }
    return ret;
}
...
+ (NSData *)encryptData:(NSData *)data withKeyRef:(SecKeyRef) keyRef{
    ...
    for(int idx=0; idx<srclen; idx+=src_block_size){
        ...
        if (status != 0) {
            WlLog(@"SecKeyEncrypt fail. Error Code: %zd", status);
            ret = nil;
            break;
        }else{
...
{
    SecKeyRef ref = [self getPublicKeyRefWithContentsOfFile:path];
    NSData *originData = [self hex2data:str];
    NSData *data = [self decryptData:originData withKeyRef:ref];
