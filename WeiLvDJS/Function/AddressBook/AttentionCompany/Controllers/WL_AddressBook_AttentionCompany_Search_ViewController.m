//
//  WL_AddressBook_AttentionCompany_Search_ViewController.m
//  WeiLvDJS
//
...
@interface WL_AddressBook_AttentionCompany_Search_ViewController ()<UITextFieldDelegate, UITableViewDelegate, UITableViewDataSource>
/** 标题栏上搜索View */
...
@implementation WL_AddressBook_AttentionCompany_Search_ViewController
static NSString *const CellId = @"searchAttentionCompanyResultCellId";
static NSString *const SearchRecordKey = @"AttentionCompanySearchRecords";
...
- (void)contentViewToSearchVc
{
    ...
    //3.添加一个提示View
    WL_AddressBook_Organiztion_Search_Prompt_View *promptView = [[WL_AddressBook_Organiztion_Search_Prompt_View alloc] initWithFrame:CGRectZero withTitle:@"企业" withIconImageName:@"WL_AddressBook_SearchCompany"];
    ...
    [searchResultTableView registerClass:[WL_AddressBook_AttentionCompany_Cell class] forCellReuseIdentifier:CellId];
...
- (void)sendRequestToSearchResultWithSearchContent:(NSString *)content
{
    ...
    [[WLHttpManager shareManager] requestWithURL:urlStr RequestType:RequestTypePost Parameters:params Success:^(id responseObject) {
        ...
        if (self.companys.count == 0 || self.companys == nil) {
            [self.alert createTip:@"没有符合条件的搜索结果"];
            self.searchResultTableView.hidden = YES;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.searchRecords containsObject:cell.companyNameLable.text])
    {
        [self.searchRecords removeObject:cell.companyNameLable.text];
    }
    else
    {
    ...
    [self.searchRecords insertObject:cell.companyNameLable.text atIndex:0];
    ...
    organizationDetailVc.company_id = cell.searchCompanyModel.ID;
