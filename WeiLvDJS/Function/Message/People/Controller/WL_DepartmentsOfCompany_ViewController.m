...
//
//  WL_DepartmentsOfCompany_ViewController.m
//  WeiLvDJS
//
...
@interface WL_DepartmentsOfCompany_ViewController ()<UITableViewDelegate,UITableViewDataSource,UISearchControllerDelegate,UISearchResultsUpdating>
...
@property(nonatomic,strong)NSMutableDictionary *originArray;
@property(nonatomic,strong)WL_NoData_View *noDataView;
@property(nonatomic,strong)UITableViewController *tableVC;

@end

@implementation WL_DepartmentsOfCompany_ViewController
...

{
    ...
    NSDictionary *paramaters =@{@"user_id":userId,@"user_password":encryptStr,@"department_id":@"0",@"company_id":self.model.company_id,@"type":@"1"};
    ...
    [[WLHttpManager shareManager]requestWithURL:UserGetMyCompanyDepartmentListUrl RequestType:RequestTypePost Parameters:paramaters Success:^(id responseObject) {
        ...
        if ([netModel.result integerValue]==1) {
            ...
            self.departmentArray =[WL_DepartmentModel mj_objectArrayWithKeyValuesArray:netModel.data[@"department_list"]];
            
            self.staffArray = [WL_StaffModel mj_objectArrayWithKeyValuesArray:netModel.data[@"staff_list"]];
            
            
            
            
            if (self.departmentArray.count==0&&self.staffArray.count==0) {
            ...
            //由于uicollectionView布局完成之后 滚动的方法才会起作用//所以-》主线程为串行，tableview刷新完毕 才会走下面，
            dispatch_async(dispatch_get_main_queue(), ^{
                
             [self.history.collectionView scrollToItemAtIndexPath:[NSIndexPath indexPathForRow:1 inSection:0]atScrollPosition:UICollectionViewScrollPositionRight animated:YES];
            });
        }
...
-(void)sureButton:(UIButton *)button
{
    ...
    if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"adressbook"] isEqualToString:@"2"]) {
...
//        NSArray *arr= self.navigationController.viewControllers;
    ...
    }else if([[[NSUserDefaults standardUserDefaults] objectForKey:@"adressbook"] isEqualToString:@"1"]){
//        NSArray *arr= self.navigationController.viewControllers;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (tableView==self.tableView) {
        ...
        if (self.type==1) {
            ...
            self.history = [[JHCusomHistory alloc] initWithFrame:CGRectMake(0,0, CGRectGetWidth(self.view.frame),55) andItems:@[@"选择接收人",self.model.company_name] andItemClickBlock:^(NSInteger i) {
            ...
            line.backgroundColor = bordColor;
            [cell.contentView addSubview:line];
            self.history = [[JHCusomHistory alloc] initWithFrame:CGRectMake(0,0, CGRectGetWidth(self.view.frame),55) andItems:@[@"选择联系人",self.model.company_name] andItemClickBlock:^(NSInteger i) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([cell isKindOfClass:[WL_CorporateStructure_Cell class]]) {
        ...
        WL_DepartmentModel *model1=[[WL_DepartmentModel alloc]init];
        
        model1.department_name =@"选择接收人";
        
        model1.company_id = @"";
        
        WL_DepartmentModel *model2=[[WL_DepartmentModel alloc]init];
        
        model2.department_name =self.model.company_name;
        
        model2.company_id = self.model.company_id;
        
        self.array =[NSMutableArray arrayWithObjects:model1,model2,model, nil];
...
-(void)popToViewControllerWith:(NSInteger )index
{
    ...
    if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"adressbook"] isEqualToString:@"1"]) {
    ...
    }else if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"adressbook"] isEqualToString:@"2"])
    {
        NSArray *arr = self.navigationController.viewControllers;
