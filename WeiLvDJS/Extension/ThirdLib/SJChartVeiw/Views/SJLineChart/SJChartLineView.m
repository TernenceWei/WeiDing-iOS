...
//        [coverView addGestureRecognizer:gesutre];
        [self gesutreActionNC:BASE_TAG_COVERVIEW + i];
    }
}
...
- (void)gesutreAction:(UITapGestureRecognizer *)sender {
    ...
    popBtn.frame = CGRectMake(point.x - 30, point.y - 20, 40, 15);
    
    [popBtn setBackgroundImage:[UIImage imageNamed:@"btg_pop_bg.png"] forState:UIControlStateNormal];//btg_pop_bg
...
- (void)gesutreActionNC:(NSInteger)sender
{
    
    NSInteger index = sender - BASE_TAG_COVERVIEW;
...
//    }
    ...
    UIButton *popBtn = [UIButton buttonWithType:(UIButtonTypeCustom)];
    popBtn.tag = index + BASE_TAG_POPBTN;
    popBtn.frame = CGRectMake(point.x - 50, point.y - 20, 100, 15);
