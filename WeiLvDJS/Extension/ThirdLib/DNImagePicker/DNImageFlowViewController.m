...
- (BOOL)seletedAssets:(ALAsset *)asset
{
    ...
    NSUInteger currentCount = [[NSUserDefaults standardUserDefaults] integerForKey:@"CarBookingSelectImageCount"];
    NSUInteger remainCount = 8 - currentCount;
    if (self.selectedAssetsArray.count >= remainCount) {//kDNImageFlowMaxSeletedNumber
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedStringFromTable(@"alertTitle", @"DNImagePicker", nil) message:[NSString stringWithFormat:@"您当前最多只能选择%zd张照片",remainCount] delegate:self cancelButtonTitle:NSLocalizedStringFromTable(@"alertButton", @"DNImagePicker", nil) otherButtonTitles:nil, nil];
        [alert show];
