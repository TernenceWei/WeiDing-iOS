...
-(void)SocialFriendCountFriend{
    ...
    WS(weakSelf);
    [WLNetworkMessageHandler requestWLFriendsCountWithResultBlock:^(WLResponseType responseType, id data, NSString *message) {
        
        dispatch_group_leave(self.group);
        if (responseType == WLResponseTypeSuccess) {
            WL_Network_Model *netModel = (WL_Network_Model *)data;
...
{
    WS(weakSelf);
    [WLNetworkMessageHandler requestCompanyListWithResultBlock:^(WLResponseType responseType, id data, NSString *message) {
        dispatch_group_leave(self.group);
        if (responseType == WLResponseTypeSuccess) {
            weakSelf.companyArray = (NSMutableArray *)data;
        }
...
{
    WS(weakSelf);
    [WLNetworkMessageHandler requestUsualFriendsListWithResultBlock:^(WLResponseType responseType, id data, NSUInteger count, NSString *message) {
        dispatch_group_leave(self.group);
        if (responseType == WLResponseTypeSuccess) {
            weakSelf.usuallyArray = (NSMutableArray *)data;
            weakSelf.usuallyNum = [NSString stringWithFormat:@"%ld", count];
        }
    }];
...
- (void)updateSearchResultsForSearchController:(UISearchController *)searchController {
    ...
    [WLNetworkMessageHandler searchFriendOrGroupWithText:searchText
                                                    type:@"1"
                                               dataBlock:^(WLResponseType responseType, id data, NSString *message) {
                                                   ...
                                                   if (responseType == WLResponseTypeSuccess) {
                                                       ...
                                                       weakSelf.tableVC = (UITableViewController *)searchController.searchResultsController;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == self.tableView){
        ...
        }else if (indexPath.section==1){//公司列表
