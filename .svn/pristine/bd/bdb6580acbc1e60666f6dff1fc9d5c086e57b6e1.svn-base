        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            if (!triedFinalizingOpenStatements) {
                ...
                while ((pStmt = sqlite3_next_stmt(_db, nil)) !=0) {
                    WlLog(@"Closing leaked statement");
...
static int FMDBDatabaseBusyHandler(void *f, int count) {
    ...
    if (delta < [self maxBusyRetryTimeInterval]) {
        ...
        if (actualSleepInMilliseconds != requestedSleepInMillseconds) {
            WlLog(@"WARNING: Requested sleep of %i milliseconds, but SQLite returned %i. Maybe SQLite wasn't built with HAVE_USLEEP=1?", requestedSleepInMillseconds, actualSleepInMilliseconds);
        }
        return 1;
    }
...
- (int)busyRetryTimeout {
    WlLog(@"%s:%d", __FUNCTION__, __LINE__);
    WlLog(@"FMDB: busyRetryTimeout no longer works, please use maxBusyRetryTimeInterval");
    return -1;
}
...
#pragma unused(i)
    WlLog(@"%s:%d", __FUNCTION__, __LINE__);
    WlLog(@"FMDB: setBusyRetryTimeout does nothing, please use setMaxBusyRetryTimeInterval:");
}
...
- (BOOL)rekeyWithData:(NSData *)keyData {
    ...
    if (rc != SQLITE_OK) {
        WlLog(@"error on rekey: %d", rc);
        WlLog(@"%@", [self lastErrorMessage]);
    }
...
- (void)warnInUse {
    WlLog(@"The FMDatabase %@ is currently in use.", self);
...
- (BOOL)databaseExists {
    ...
    if (!_db) {
        ...
        WlLog(@"The FMDatabase %@ is not open.", self);
...
- (FMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (!pStmt) {
        ...
        if (SQLITE_OK != rc) {
            if (_logsErrors) {
                WlLog(@"DB Error: %d \"%@\"", [self lastErrorCode], [self lastErrorMessage]);
                WlLog(@"DB Query: %@", sql);
                WlLog(@"DB Path: %@", _databasePath);
            }
    ...
    if (dictionaryArgs) {
        ...
        for (NSString *dictionaryKey in [dictionaryArgs allKeys]) {
            ...
            if (_traceExecution) {
                WlLog(@"%@ = %@", parameterName, [dictionaryArgs objectForKey:dictionaryKey]);
            }
            ...
            if (namedIdx > 0) {
                ...
                WlLog(@"Could not find index for %@", dictionaryKey);
            }
        }
    }
    else {
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (closeErrorCode != SQLITE_OK) {
        if (_logsErrors) {
            WlLog(@"Unknown error finalizing or resetting statement (%d: %s)", closeErrorCode, sqlite3_errmsg(_db));
            WlLog(@"DB Query: %@", sql);
        }
    }
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(FMDBExecuteStatementsCallbackBlock)block {
    ...
    if (errmsg && [self logsErrors]) {
        WlLog(@"Error inserting batch: %s", errmsg);
        sqlite3_free(errmsg);
    }
