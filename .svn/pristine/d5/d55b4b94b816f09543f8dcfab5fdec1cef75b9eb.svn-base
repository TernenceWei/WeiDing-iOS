...
/** 我的车辆控制器 */
#import "WL_Application_Driver_MyCar_ViewController.h"
...
@interface WL_ApplicationViewController ()<UITableViewDelegate, UITableViewDataSource, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, EScrollerViewDelegate>
/** 应用模块头部切换公司/组织的View */
@property(nonatomic, weak)WL_Application_Title_View *applicationTitleView;
/** 选择公司/组织的下拉列表 */
...
/**< 叫车应用标题数组 */
@property (nonatomic, strong) NSArray *bookCarArr;
/**< 店铺应用图片数组 */
...
//@property(nonatomic, strong) WL_NoData_View *;
...
//                self.homePageNoDataImage.hidden = NO;
                self.noDataView.tipLabel.text = @"您尚未加入任何企业";
                self.noDataView.hidden = NO;
                self.applicationTitleView.organizationLable.text = @"微叮";
                self.applicationTitleView.hidden = NO;
                [self hidHud];
                return;
...
//                 self.company_id = self.currentCompanyModel.company_id;
            ...
            self.applicationTitleView.organizationLable.text = self.currentCompanyModel.company_name;
//
//            if (self.currentCompanyModel.role.count > 0) {
//             
////                self.noRoleImage.hidden = YES;
...
- (void)sendRequestToDriverStatue
{
    ...
    [[WLNetworkDriverHandler sharedInstance] requestDriverAndCarStatusWithResultBlock:^(BOOL success, WLDriverStatus driverStatus, WLCarStatus carStatus,NSNumber *drive_id, NSString *message) {
        if (success) {
...
- (void)setupArrayToApplication
{
    ...
    //店铺应用标题数组
    self.storeArr = @[@"店铺"];
    self.storeImageArr = @[@"Cancel-after-verification"];
    
    //叫车应用标题数组
    self.bookCarArr = @[ @"叫车",@"我的订单" ];
    self.bookCarImageArr = @[@"shouyejiedan",@"wdeshouru"];
...
- (void)setupContentToApplication
{
    ...
    //没有加入任何组织的imageView
//    UIImageView *homePageNoDataImage = [[UIImageView alloc] init];
//    [self.view addSubview:homePageNoDataImage];
//    [homePageNoDataImage mas_makeConstraints:^(MASConstraintMaker *make) {
//        make.left.equalTo(self.view.mas_left);
//        make.right.equalTo(self.view.mas_right);
//        make.top.equalTo(self.view.mas_top).offset(-64);
//        make.bottom.equalTo(self.view.mas_bottom).offset(49);
//    }];
//    homePageNoDataImage.image = [UIImage imageNamed:@"homePageNoData"];
//    self.homePageNoDataImage = homePageNoDataImage;
//    self.homePageNoDataImage.hidden = YES;
    
    //没有角色的imageView
//    UIImageView *noRoleImage = [[UIImageView alloc] init];
//    [self.view addSubview:noRoleImage];
//    [noRoleImage mas_makeConstraints:^(MASConstraintMaker *make) {
//        make.left.equalTo(self.view.mas_left);
//        make.right.equalTo(self.view.mas_right);
//        make.top.equalTo(self.view.mas_top).offset(-64);
//        make.bottom.equalTo(self.view.mas_bottom).offset(49);
//    }];
//    noRoleImage.image = [UIImage imageNamed:@"Application_Home_NoRole"];
//    self.noRoleImage = noRoleImage;
//    self.noRoleImage.hidden = YES;
...
- (void)setupNavToApplication
{
    ...
    //初始化(切换公司/组织)标题View
    WL_Application_Title_View *applicationTitleView = [[WL_Application_Title_View alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 44)];
    
    //设置为应用页面的TitleView
    self.navigationItem.titleView = applicationTitleView;
    
    applicationTitleView.hidden = YES;
    
    [applicationTitleView.organizationLable sizeToFit];
    //赋值给全局变量
    self.applicationTitleView = applicationTitleView;
    
    
    //添加标题View的指示器按钮点击方法
    [applicationTitleView.sliderButton addTarget:self action:@selector(selectedOrganizationClick) forControlEvents:UIControlEventTouchUpInside];
    
    //添加单击手势在titleView上
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(selectedOrganizationClick)];
    [applicationTitleView addGestureRecognizer:tap];
...
{
    //指示器按钮选中状态取反
    self.applicationTitleView.sliderButton.selected = !self.applicationTitleView.sliderButton.selected;
    
    //如果是选中状态
    if (self.applicationTitleView.sliderButton.selected == YES)
    {
...
- (void)coverViewClick
{
    self.coverView.hidden = YES;
    self.organizationListTableView.hidden = YES;
    self.applicationTitleView.sliderButton.selected = NO;
}
...
//    cell.company = self.currentCompanyModel;
    ...
    if ([cell.organizationNameLable.text isEqualToString:self.applicationTitleView.organizationLable.text])
    {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    self.applicationTitleView.organizationLable.text = cell.organizationNameLable.text;
    self.applicationTitleView.sliderButton.selected = NO;
    self.organizationListTableView.hidden = YES;
    self.coverView.hidden = YES;
    //切换当前选中的公司模型
...
//        self.noRoleImage.hidden = NO;
    ...
    {
//        [self hideNoData:YES andType:WLNetworkTypeNOData];
        collectionView.hidden = NO;
//        self.noRoleImage.hidden = YES;
        return self.contentRoles.count + 1;//因为个人叫车是一直都有的
    }
...
{
    if(section<self.contentRoles.count)
    {
        ...
        else if ([role.type isEqualToString:@"3"] ||[role.type isEqualToString:@"5"]||[role.type isEqualToString:@"9"])
        {
            return 3;
        }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.contentRoles.count > 0 && self.contentRoles != nil)
    {
        ...
        //取出角色数组角色模型
        if(indexPath.section<self.contentRoles.count)
        {
            ...
            }else if([role.type isEqualToString:@"9"])//企业叫车
            {
                ...
                cell.functionLable.text = self.bookCarArr[indexPath.item];
                cell.iconImageView.image = [UIImage imageNamed:self.bookCarImageArr[indexPath.item]];
                [self hideStatusToCell:cell];
            }
        }else if(indexPath.section == self.contentRoles.count)//个人叫车
        {
            if (indexPath.item == self.storeArr.count+1) {
                cell.maskView.hidden = NO;
                return cell;
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout referenceSizeForFooterInSection:(NSInteger)section
{
    if (section < self.contentRoles.count) {
        WL_Application_Role_Model *role = self.contentRoles[section];
        if ([role.type isEqualToString:@"5"] ||[role.type isEqualToString:@"100"]||[role.type isEqualToString:@"9"]) {
...
{
    //如果公司角色数组有数据
    if (self.contentRoles.count > 0 && self.contentRoles != nil&&indexPath.section<self.contentRoles.count)
    {
        //取出角色数组第一个角色模型
        WL_Application_Role_Model *role = self.contentRoles[indexPath.section];
        ...
        }else if([role.type isEqualToString:@"100"])
        {
            ...
            {
//                WLStoreViewController *scanVC = [[WLStoreViewController alloc]init];
//                scanVC.hidesBottomBarWhenPushed = YES;
//                scanVC.urlString = self.storeUrl;
//                [self.navigationController pushViewController:scanVC animated:YES];
