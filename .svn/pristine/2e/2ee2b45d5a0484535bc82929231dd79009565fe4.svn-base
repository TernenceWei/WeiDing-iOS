...
-(NSString*)someDayDateStr:(NSDate *)date{
    
    NSDateComponents* firstDayOfMonth=[self componentOfDate:date];
//    if(_curDateStr==nil){
    
        NSString *dateStr =[NSString stringWithFormat:@"%ld-%ld-%ld",(long)firstDayOfMonth.year,(long)firstDayOfMonth.month,(long)firstDayOfMonth.day];
    //}
    return dateStr;
}
...
-(NSDictionary*)calendarMapWith:(NSDate*)date{
    ...
    if(firstDayOfMonth.weekday!=2){
        ...
        for(int i=0;i<weekGap;i++){
            ...
            if (item.dateStr) {
                ...
                item.weekDay = [self getWeekDayFromNsdate:item.dateStr];
    ...
    if(lastDayOfMonth.weekday!=1){
        NSInteger weekGap=7-lastDayOfMonth.weekday;
...
-(NSString *)getWeekDayFromNsdate:(NSString *)dateString
...

{
    ...
    if (![WLTools judgeString:currentDateString]) {
