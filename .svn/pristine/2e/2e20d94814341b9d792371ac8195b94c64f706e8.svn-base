...
//
//  WLGuideInvalidOrderTableViewCell.m
//  WeiLvDJS
//
...
@interface WLGuideInvalidOrderTableViewCell ()
/** 是否为主导游 */
...
@implementation WLGuideInvalidOrderTableViewCell
...
-(void)setUpBaseView{
    ...
    [statusImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(baseView.mas_right).offset(-8);
        make.top.equalTo(baseView.mas_top);
    }];

    if(self.orderInfo.checkListID.intValue == 4){
        [statusImageView setImage:[UIImage imageNamed:@"denied"]];
    }
    else
    {
        [statusImageView setImage:[UIImage imageNamed:@"revoked"]];
    }
