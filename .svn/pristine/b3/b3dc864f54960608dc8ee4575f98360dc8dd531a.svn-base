//
//  WL_ResetPayPassword_ViewController.m
//  WeiLvDJS
//
...
@interface WL_ResetPayPassword_ViewController ()<UITextFieldDelegate>
...
@implementation WL_ResetPayPassword_ViewController
...
-(void)createUI
{
   ...
   // [codeTextField addTarget:self action:@selector(textChange:) forControlEvents:UIControlEventEditingChanged];
    ...
    UILabel *identityLabel =[WLTools allocLabel:@"身份证后8位" font:WLFontSize(14) textColor:BlackColor frame:CGRectMake(10,ViewBelow(codeLabel), 100, 45) textAlignment:NSTextAlignmentLeft];
    [backGroundView addSubview:identityLabel];
    
    
    identityTextField =[[UITextField alloc]initWithFrame:CGRectMake(ViewX(codeTextField), ViewBelow(codeLabel), ScreenWidth-100-10, 45)];
    
    identityTextField.font =WLFontSize(14);
    
    identityTextField.delegate = self;
    
    identityTextField.clearButtonMode = UITextFieldViewModeWhileEditing;
    
    identityTextField.keyboardType = UIKeyboardTypeNumberPad;
    
    [identityTextField addTarget:self action:@selector(textChange:) forControlEvents:UIControlEventEditingChanged];
    
    [backGroundView addSubview:identityTextField];
...
-(void)confirmButtonClick
{
    ...
    if (identityTextField.text.length==0) {
        ...
        [[WL_TipAlert_View sharedAlert]createTip:@"请输入身份证后8位"];
...
-(void)goNext
{
    ...
    NSDictionary *dict =@{@"user_id":user_id,@"user_password":encryptStr,@"type":@"1",@"code":codeTextField.text,@"idCard":@"07105010"};
...
-(void)textChange:(UITextField *)textField
{
    ...
    if (textField==identityTextField) {
        ...
        if (identityTextField.text.length>=8) {
            
            identityTextField.text =[identityTextField.text substringToIndex:8];
