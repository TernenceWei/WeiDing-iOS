...
#import "WLRegularExpression.h"

@implementation WLRegularExpression
...
{
    return [WLRegularExpression ba_isChinaMobile:phoneNum]? @"中国移动": ([WLRegularExpression ba_isChinaUnicom:phoneNum]? @"中国联通":([WLRegularExpression ba_isChinaTelecom:phoneNum]? @"中国电信": @"未知号码"));
}
...
#pragma mark - ***** 判断输入金额正确与否
+ (BOOL)ba_isMoneyNumberString:(NSString *)string
{
    NSString *moneyNumber = @"^[0-9]+(\\.[0-9]{1,2})?$";
    NSPredicate *taxNoPredicate = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",moneyNumber];
    
    return [taxNoPredicate evaluateWithObject:string];
}
@end
