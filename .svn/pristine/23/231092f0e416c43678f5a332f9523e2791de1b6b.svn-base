...
-(NSDate *)date
{
    ...
    NSString *year = [self.years objectAtIndex:0];
    ...
    NSString *hour;
    if (self.datePickerMode == DatePickerHourMode || self.datePickerMode == DatePickerTimeMode) {
        hour = [self.hours objectAtIndex:([self selectedRowInComponent:HOUR])];
    }
    
    NSString *minute;
    if (self.datePickerMode == DatePickerTimeMode) {
        minute = [self.minutes objectAtIndex:([self selectedRowInComponent:MINUTE])];
    }
    ...
    } else if(self.datePickerMode == DatePickerDateMode)
    {
        ...
        [formatter setDateFormat:@"yyyy年M月d日H时m分"];
        date = [formatter dateFromString:[NSString stringWithFormat:@"%@%@%@%@%@", year, month, day, hour,minute]];
        if (!date && [hour isEqual:@"0时"]) {
            [formatter setDateFormat:@"yyyy年M月d日"];
            date = [formatter dateFromString:[NSString stringWithFormat:@"%@%@%@", year, month, day]];
        }
...
-(UIView *)pickerView: (UIPickerView *)pickerView viewForRow: (NSInteger)row forComponent: (NSInteger)component reusingView: (UIView *)view
{
    ...
    else if(component == MINUTE)
    {
        ...
        NSString *minuteName = [self.minutes objectAtIndex:(row % minuteCount)];
        NSString *currenrMinuteName  = [self currentMinuteName];
        if([minuteName isEqualToString:currenrMinuteName] == YES)
        {
            selected = YES;
        }
    }
...
-(NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component
{
    if(component == MONTH){
        ...
        return [self bigRowMinutesCount];
    }
}
...
-(NSArray *)nameOfHours
{
    ...
    if (self.datePickerMode == DatePickerHourMode) {
        [hours addObject:@""];
        for(NSInteger hour = 1; hour <= 23; hour++)
        {
...
-(NSArray *)nameOfMinutes
{
    ...
    for(NSInteger hour = 0; hour <= 3; hour++)
    {
        NSString *hourStr = [NSString stringWithFormat:@"%li分", (long)hour * 15];
        [hours addObject:hourStr];
    }
    return hours;
}
...
-(NSArray *)nameOfMinHours
{
    ...
    NSInteger minMin = [formatter1 stringFromDate:self.minDate].integerValue;
    if (minMin > 45) {
        hourMin = hourMin + 1;
    }
...
-(NSArray *)nameOfMaxHours
{
    ...
    if (self.datePickerMode == DatePickerHourMode) {
        [hours addObject:@""];
        for(NSInteger hour = 1; hour <= hourMax; hour++)
        {
            NSString *hourStr = [NSString stringWithFormat:@"%li时", (long)hour];
            [hours addObject:hourStr];
        }
...
-(NSArray *)nameOfMinMinutes
{
    ...
    NSInteger hourMin = [formatter stringFromDate:self.minDate].integerValue;
    NSInteger index = 0;
    if (hourMin >= 0 && hourMin <= 15) {
        index = 1;
    }else if (hourMin > 15 && hourMin <= 30){
        index = 2;
    }else if (hourMin > 30 && hourMin <= 45){
        index = 3;
    }else if (hourMin > 45 && hourMin < 59){
        index = 0;
    }
    for(NSInteger hour = index; hour <= 3; hour++)
    {
        NSString *hourStr = [NSString stringWithFormat:@"%li分", (long)hour * 15];
        [hours addObject:hourStr];
    }
...
-(NSArray *)nameOfMaxMinutes
{
    ...
    NSInteger hourMax = [formatter stringFromDate:self.maxDate].integerValue;
    NSInteger index = 0;
    if (hourMax >= 0 && hourMax <= 15) {
        index = 1;
    }else if (hourMax > 15 && hourMax <= 30){
        index = 2;
    }else if (hourMax > 30 && hourMax <= 45){
        index = 3;
    }else if (hourMax > 45 && hourMax < 59){
        index = 0;
    }
    for(NSInteger hour = 0; hour <= index; hour++)
    {
        NSString *hourStr = [NSString stringWithFormat:@"%li分", (long)hour * 15];
        [hours addObject:hourStr];
    }
...
//        }else
    ...
    if (self.datePickerMode != DatePickerYearMonthMode) {
        ...
        if (component == 0 ||component == 1) {
            ...
            } else if ([self selectedRowInComponent:MONTH] == self.months.count - 1 ) {
                self.days = [self nameOfMaxDays];
            } else if ([self selectedRowInComponent:MONTH] == 0 ) {
    ...
    if (self.datePickerMode == DatePickerHourMode || self.datePickerMode == DatePickerTimeMode) {
        if (component != HOUR) {
            if (([self selectedRowInComponent:MONTH] == self.months.count - 1) && ([self selectedRowInComponent:DAY] == self.days.count - 1)) {
                self.hours = [self nameOfMaxHours];
                
            } else if ([self selectedRowInComponent:MONTH] == 0  && [self selectedRowInComponent:DAY] == 0) {
    ...
    if (self.datePickerMode == DatePickerTimeMode) {
        if (component != MINUTE) {
            if (([self selectedRowInComponent:MONTH] == self.months.count - 1) && ([self selectedRowInComponent:DAY] == self.days.count - 1)&& ([self selectedRowInComponent:HOUR] == self.hours.count - 1)) {
                self.minutes = [self nameOfMaxMinutes];
                
            } else if ([self selectedRowInComponent:MONTH] == 0  && [self selectedRowInComponent:DAY] == 0 && [self selectedRowInComponent:HOUR] == 0) {
                self.minutes = [self nameOfMinMinutes];
...
- (void)selectDate:(NSDate *)date{
    ...
    [self selectRow: selectIndexPath.section
        inComponent: MONTH
           animated: YES];
    [self pickerView:self didSelectRow:selectIndexPath.section inComponent:MONTH];
    
    if (self.datePickerMode != DatePickerYearMonthMode) {
        CGFloat month = [self selectMonthIndexWithDate:date];
        [self selectRow:month inComponent:MONTH animated:YES];
        [self pickerView:self didSelectRow:month inComponent:MONTH];
    ...
    if (self.datePickerMode == DatePickerHourMode || self.datePickerMode == DatePickerTimeMode) {
        int hour = [self selectHourIndexWithDate:date];
        [self selectRow:hour inComponent:HOUR animated:YES];
        [self pickerView:self didSelectRow:hour inComponent:HOUR];
        if (self.datePickerMode == DatePickerHourMode && hour == 0) {
            [self selectRow:0 inComponent:HOUR animated:YES];
            [self pickerView:self didSelectRow:0 inComponent:HOUR];
        }
    }
    if (self.datePickerMode == DatePickerTimeMode) {
        CGFloat minute = [self selectMinuteIndexWithDate:date];
        [self selectRow:minute inComponent:MINUTE animated:YES];
        [self pickerView:self didSelectRow:minute inComponent:MINUTE];
        
        NSArray *array;
        if ([self selectedRowInComponent:HOUR] == 0) {
            array = [self nameOfMinMinutes];
        }else{
            array = [self nameOfMinutes];
        }
        
        NSUInteger index = minute - (4 - array.count);
            
        [self selectRow:index  inComponent:MINUTE animated:YES];
        [self pickerView:self didSelectRow:index inComponent:MINUTE];
...
-(CGFloat)selectMonthIndexWithDate:(NSDate *)date {// row - month ; section - year
    CGFloat index = 0.f;
    NSString *Day = [self selectMonthName:date];
    for (NSString *cellDay in self.months)
    {
        if([cellDay isEqualToString:Day])
        {
            index = [self.months indexOfObject:cellDay];
...
-(CGFloat)selectMinuteIndexWithDate:(NSDate *)date {// row - month ; section - year
    CGFloat index = 0.f;
    NSString *Hour = [self selectMinuteName:date];

    NSInteger hourMax = Hour.integerValue;
    if (hourMax >= 0 && hourMax < 15) {
        index = 0;
    }else if (hourMax >= 15 && hourMax < 30){
        index = 1;
...
- (NSDate *)extractMinuteDate:(NSDate *)date {
    //get seconds since 1970
    NSTimeInterval interval = [date timeIntervalSince1970];
    int daySeconds = 60 * 60;
    //calculate integer type of days
