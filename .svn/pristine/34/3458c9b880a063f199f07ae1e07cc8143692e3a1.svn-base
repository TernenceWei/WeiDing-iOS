...
-(void)setLayout
{
    UIView *dismissView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, SCREEN_HEIGHT - 180)];
    [self addSubview:dismissView];
    UITapGestureRecognizer *tapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tapGestureAction)];
    [dismissView addGestureRecognizer:tapGesture];
    
    self.nameArray =[NSMutableArray arrayWithObjects:@"中国工商银行",@"中国建设银行",@"招商银行",@"中国银行",@"广发银行", @"浦发银行",@"平安银行",@"农业银行",@"中国邮政储蓄银行",@"中信银行",@"北京银行",@"广大银行",@"兴业银行",@"民生银行",nil];
