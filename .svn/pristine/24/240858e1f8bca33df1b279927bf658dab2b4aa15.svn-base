 ...
 // [UIApplication sharedApplication].statusBarStyle =
  ...
  [[RCRealTimeLocationManager sharedManager]
      ...
      error:^(RCRealTimeLocationErrorCode status) {
        WlLog(@"get location share failure with code %d", (int)status);
      }];
...
//                                        tag:PLUGIN_BOARD_ITEM_FILE_TAG];
    ...
    [self.pluginBoardView insertItemWithImage:[UIImage imageNamed:@"2"]
                                        title:@"1"
                                      atIndex:0
                                          tag:101];
//
...
- (void)notifyUpdateUnreadMessageCount {
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    UIImageView *backImg = [[UIImageView alloc]
        initWithImage:[UIImage imageNamed:@"NavigationBack"]];
    backImg.frame = CGRectMake(-6, 0, 12, 22);
...
- (RCMessageBaseCell *)rcConversationCollectionView:
  ...
  if ([messageContent isMemberOfClass:[RCRealTimeLocationStartMessage class]]) {
   ...
   // __cell.backgroundColor = [UIColor redColor];
