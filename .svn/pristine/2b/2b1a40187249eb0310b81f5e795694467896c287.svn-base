...
+ (NSArray *)jsonArrayWithObjectArray:(NSArray *)objectArray {
    ...
    for (WLBaseModel *model in objectArray) {
...
+ (NSString *)jsonStringWithObjectArray:(NSArray *)objectArray {
    ...
    if (error) {
        WlLog(@"Error occured when create json string with object array: %@, error: %@", objectArray, error.localizedDescription);
    }
...
- (id)initWithJsonString:(NSString *)json {
    ...
    if (self) {
        ...
        if (error) {
            WlLog(@"Error occured when init object with json string: %@, error: %@", json, error.localizedDescription);
            return nil;
        }
        ...
        if ([self respondsToSelector:setter]) {
            ...
            if ([aDictValue isKindOfClass:[NSDictionary class]]) {
                ...
                if ([type isEqualToString:@"NSDictionary"] == NO && [type isEqualToString:@"NSMutableDictionary"] == NO) {
                    ...
                    if ([newClass isSubclassOfClass:[WLBaseModel class]]) {
                ...
                if (classString) {
                    for (id content in aDictValue) {
                        ...
                        //如果是该类的子类，说明能响应initWithDict:方法并且能建立映射字典。
                        if ([classInArray isSubclassOfClass:[WLBaseModel class]]) {
...
- (NSDictionary *)dictionaryRepresentation
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (key && value)
        {
            if ([value isKindOfClass:[WLBaseModel class]])
            {
...
- (NSString *)jsonStringRepresentation
{
    ...
    if (error)
    {
        WlLog(@"Error occured when create json string representation, error: %@", error.localizedDescription);
    }
