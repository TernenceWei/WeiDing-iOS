...
- (void)setUpToolBars
{
    ...
    deleteButton.frame = CGRectMake(self.bounds.size.width - 60, 0, 50, 30);
    deleteButton.xl_centerY = 35;
...
- (void)setUpBrowserStyle
{
    switch (self.browserStyle) {
        ...
        case XLPhotoBrowserStyleSimple:
        {
            ...
            break;
        case XLPhotoBrowserStyleCarBooking:
        {
...
- (void)deleteImage
{
    if ([self.delegate respondsToSelector:@selector(photoBrowser:deleteImage:)]) {
        [self.delegate photoBrowser:self deleteImage:self.currentImageIndex];
    }
    [self.images removeObjectAtIndex:self.currentImageIndex];
